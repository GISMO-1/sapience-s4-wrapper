...
export const consumer: Consumer = kafka.consumer({ groupId: "integration-service-group" });
...
async function connectWithRetry(action: () => Promise<void>, label: string): Promise<void> {
  ...
  while (true) {
    try {
      await action();
      logger.info({ label }, "Connected to Kafka");
      return;
    } catch (error) {
      logger.warn({ error, label, delay }, "Kafka connection failed; retrying");
